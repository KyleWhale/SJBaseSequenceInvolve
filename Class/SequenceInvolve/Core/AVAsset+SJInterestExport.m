...
- (void)exportWithStartTime:(NSTimeInterval)startTime
    ...
    AVMutableCompositionTrack *trackM2 = [compositionM addMutableTrackWithMediaType:AVMediaTypeVideo preferredTrackID:kCMPersistentTrackID_Invalid];
    ...
    AVAssetTrack *assetTrack2 = [asset tracksWithMediaType:AVMediaTypeVideo].firstObject;
    NSError *error = nil;
    [trackM insertTimeRange:cutRange ofTrack:assetTrack atTime:kCMTimeZero error:&error];
    if ( error ) { NSLog(@"Export Failed: error = %@", error); if ( failure ) failure(self.asset, error); return;}
    [trackM2 insertTimeRange:cutRange ofTrack:assetTrack2 atTime:kCMTimeZero error:&error];
    if ( error ) { NSLog(@"Export Failed: error = %@", error); if ( failure ) failure(self.asset, error); return;}
    trackM2.preferredTransform = assetTrack2.preferredTransform;
...
@implementation AVAsset (SJInterestExport)
#pragma mark -
