//
//  SJRetrieveRetrievePresentView.m
...
@interface SJRetrieveRetrievePresentView ()<UIGestureRecognizerDelegate>
...
@implementation SJRetrieveRetrievePresentView
...
- (void)handleSingleTap:(UITouch *)tap {
    if ( ![self _isGestureSupported:SJAccidentPresenceGestureTypeMask_SingleTap ] )
        return;
    
    CGPoint location = [tap locationInView:self];
    if ( _gestureRecognizerShouldTrigger && _gestureRecognizerShouldTrigger(self, SJAccidentPresenceGestureType_SingleTap, location) ) {
...
- (void)handleDoubleTap:(UITouch *)tap {
    if ( ![self _isGestureSupported:SJAccidentPresenceGestureTypeMask_DoubleTap] )
        return;
    
    CGPoint location = [tap locationInView:self];
    if ( _gestureRecognizerShouldTrigger && _gestureRecognizerShouldTrigger(self, SJAccidentPresenceGestureType_DoubleTap, location) ) {
...
- (void)cancelGesture:(SJAccidentPresenceGestureType)type {
    ...
    switch ( type ) {
        default: break;
        case SJAccidentPresenceGestureType_Pan:
            gesture = _pan;
            break;
        case SJAccidentPresenceGestureType_Pinch:
            gesture = _pinch;
            break;
        case SJAccidentPresenceGestureType_LongPress:
            gesture = _longPress;
            break;
    }
...
- (void)setPlaceholderImageViewHidden:(BOOL)placeholder animated:(BOOL)animated {
    if ( placeholder ) {
...
- (void)_setupViews {
    self.supportedGestureTypes = SJAccidentPresenceGestureTypeMask_Default;
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    SJAccidentPresenceGestureType type = SJAccidentPresenceGestureType_Pan;
    if ( gestureRecognizer == _pinch )
        type = SJAccidentPresenceGestureType_Pinch;
    else if ( gestureRecognizer == _longPress )
        type = SJAccidentPresenceGestureType_LongPress;
    
    switch ( type ) {
        default: break;
        case SJAccidentPresenceGestureType_Pan: {
            if ( ![self _isGestureSupported:SJAccidentPresenceGestureTypeMask_Pan] )
                return NO;
            ...
            if ( _movingDirection == SJPanGestureMovingDirection_H && ![self _isGestureSupported:SJAccidentPresenceGestureTypeMask_Pan_H] )
                return NO;
            
            if ( _movingDirection == SJPanGestureMovingDirection_V && ![self _isGestureSupported:SJAccidentPresenceGestureTypeMask_Pan_V] )
                return NO;
            ...
            break;
        case SJAccidentPresenceGestureType_Pinch: {
            if ( ![self _isGestureSupported:SJAccidentPresenceGestureTypeMask_Pinch] )
                return NO;
        }
            break;
        case SJAccidentPresenceGestureType_LongPress: {
            if ( ![self _isGestureSupported:SJAccidentPresenceGestureTypeMask_LongPress] )
                return NO;
        }
            break;
    }
...
- (BOOL)_isGestureSupported:(SJAccidentPresenceGestureTypeMask)type {
