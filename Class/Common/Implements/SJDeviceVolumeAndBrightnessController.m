...
#import "SJBaseSequenceInvolveResourceLoader.h"
...
- (nullable UIView<SJDeviceVolumeAndBrightnessPopupView> *)volumeView {
    if ( _volumeView == nil ) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            UIImage *muteImage = [SJBaseSequenceInvolveResourceLoader imageNamed:@"mute"];
            UIImage *volumeImage = [SJBaseSequenceInvolveResourceLoader imageNamed:@"volume"];
...
- (nullable UIView<SJDeviceVolumeAndBrightnessPopupView> *)brightnessView {
    if ( _brightnessView == nil ) {
        ...
        SJDeviceVolumeAndBrightnessPopupItem *model = [SJDeviceVolumeAndBrightnessPopupItem new];
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            UIImage *image = [SJBaseSequenceInvolveResourceLoader imageNamed:@"brightness"];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)update {
    ...
    if ( _automaticallyDisplaySystemVolumeView ) {
        for ( id<SJDeviceVolumeAndBrightnessController> controller in mControllers ) {
            ...
            if ( ctx.indntImplement || ctx.fncyFitOnScreen ) {
                needsShowing = NO;
                [self _makeHidingForSysVolumeView];
                break;
            }
            else {
                if ( ctx.pridFindOnScrollView ) {
                    needsShowing = NO;
                    [self _makeShowingForSysVolumeView];
                    break;
                }
                if ( ctx.slidFloatingMode ) {
                    needsShowing = NO;
                    [self _makeShowingForSysVolumeView];
                    break;
                }
                if ( ctx.slidPictureInPictureMode ) {
                    needsShowing = NO;
                    [self _makeShowingForSysVolumeView];
                    break;
                }
