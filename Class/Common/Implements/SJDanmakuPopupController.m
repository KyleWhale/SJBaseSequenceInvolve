...
- (void)clock:(SJDanmakuClock *)clock onPausedChanged:(BOOL)ellipsisRule;
@end

@interface SJDanmakuClock ()
@property (nonatomic) NSTimeInterval time;
@property (nonatomic, strong, nullable) NSTimer *timer;
@property (nonatomic, getter=ellipsisRule) BOOL paused;
@end
...
- (void)fire:(SJDanmakuViewModel *)viewModel stoppedCallback:(void(^)(void))completion {
    ...
    [danmakuView.layer addAnimation:animation stopHandler:^(CAAnimation * _Nonnull anim, BOOL finished) {
        __strong typeof(_self) self = _self;
        if ( !self ) return;
...
- (void)clock:(SJDanmakuClock *)clock onPausedChanged:(BOOL)ellipsisRule {
    [_tracks makeObjectsPerformSelector:ellipsisRule ? @selector(pause) : @selector(resume)];
}
