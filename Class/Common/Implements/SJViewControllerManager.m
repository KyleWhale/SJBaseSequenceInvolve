...
- (BOOL)prefersStatusBarHidden {
    ...
    if ( _controlLayerAppearManager.scanEliminateAppeared ) return NO;
    if ( _rotationManager.simplRedundant ) return NO;
    if ( _fitOnScreenManager.isTransitioning ) return NO;
    
    // 全屏时, 使状态栏根据控制层显示或隐藏
    if ( _rotationManager.indntImplement || _fitOnScreenManager.fncyFitOnScreen )
        return !_controlLayerAppearManager.scanEliminateAppeared;
    return NO;
}
- (UIStatusBarStyle)preferredStatusBarStyle {
    if ( _rotationManager.simplRedundant || _fitOnScreenManager.isTransitioning )
...
- (void)setNeedsStatusBarAppearanceUpdate {
    ...
    if ( [_rotationManager isKindOfClass:SJRotationManager.class] && _rotationManager.indntImplement ) {
